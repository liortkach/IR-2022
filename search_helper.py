...
def cosine_similarity(D, Q, doc_norm, cosineWeight=0.333):
    """
    ...
    for doc_id, row in D.iterrows():
        ...
        cosine_sim_dict[doc_id] = (top / (float(di_size) * float(q_size))) * cosineWeight
...
def get_topN_score_for_queries(query, body_index, DL, doc_norm, N=3, cosineWeight=0.333):
    """
    ...
    cosine_sim_dict = cosine_similarity(D, Q, doc_norm, cosineWeight)
